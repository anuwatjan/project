...
$columns = array('customer_name', 'customer_last','customer_phone','customer_address','customer_drug');
...
if(isset($_POST["search"]["value"]))
{
 ...
 OR customer_drug LIKE "%'.$_POST["search"]["value"].'%" 
 ';
}
...
while($row = mysqli_fetch_array($result))
{
 ...
 $sub_array[] = '<div contenteditable class="update" data-customer_id="'.$row["customer_id"].'" data-column="customer_address">' . $row["customer_address"] . '</div>';
 ...
 $sub_array[] = '<button type="button "  name="delete" class="delete btn btn-danger btn-sm" style="border:none;" customer_id="'.$row["customer_id"].'">ลบ</button>';
 $data[] = $sub_array; 
}
